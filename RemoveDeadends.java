...
public class RemoveDeadends {
	...
	public static void job(Configuration conf) throws IOException, ClassNotFoundException, InterruptedException{
		...
		while(existDeadends)
		{
			...
			job.setMapOutputKeyClass(Text.class);    // specify type of map keys
			job.setMapOutputValueClass(Text.class);  // specify type of map values

			job.setMapperClass(Map.class);     // specify the mapper that is going to be executed
			job.setReducerClass(Reduce.class);  // specify the reducer that is going to be executed

			job.setInputFormatClass(TextInputFormat.class);   // specify how to read the input file "line by  line"
			job.setOutputFormatClass(TextOutputFormat.class);  // specify how to write in the output file "line by line"
			
			FileInputFormat.setInputPaths(job, new Path(currentInput));  // specify the input file
			FileOutputFormat.setOutputPath(job, new Path(intermediaryDir)); // specify the output file
			job.waitForCompletion(true);  // launch the job
